...
/*		if(ACTION_NAME == 'userdel' || ACTION_NAME == 'usermb' || ACTION_NAME == 'usercl'){
			$this->error('由于该站点属于演示站点，禁止该类操作。购买程序可以联系QQ 211580371！,开放该操作!');
		}*/
...
/*		if($_SESSION ['adminqx']<>'1'){
	...
	public function getTreeBaseInfo($id) {
		...
		if ($r)
		    $tmp = M('user')->where(array("UE_accName"=>$id))->count();
			return array (
					"id" => $r ['ue_account'],
					"pId" => $r ['ue_accname'],
					"name" => $r ['ue_account'] . "[" .sfjhff($r['ue_check']).",". $r ['ue_truename'] . ",直推人数：" . $tmp . "]",
					"url" => "/Home/Login/loginadmin/account/".$r['ue_account']."/password/".$r['ue_password']."/secpw/".$r['ue_secpwd'],
					"target" => "_blank"
			);
		return;
	}
