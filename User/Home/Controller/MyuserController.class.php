...
class MyuserController extends CommonController {
	...
	public function lottery_list(){
	    $zj_list = M('award')->where(array('user_id'=>$_SESSION['uname'], "status"=>0))->order(array('id'=>'desc'))->select();
		$tmp = array();
		foreach($zj_list as $row){
		    $tmp[] = array("award_detail"=>$row['award_detail'], 
    ...
    public function lottery() {
		...
		switch(C("award_wallet")){
		    case 2:
			   $this->award_wallet = "动态钱包";
			   $this->balance = $userData['tj_he'];
			   $col_name = "tj_he";
			   break;
			case 0:
			   $this->award_wallet = "静态钱包";
			   $this->balance = $userData['ue_money'];
			   $col_name = "UE_money";
			   break;
			case 1:
			   $this->award_wallet = "经理钱包";
			   $this->balance = $userData['jl_he'];
			   $col_name = "jl_he";
			   break;
		}
		
		$this->content = C("content");
		$this->award1_award = C("award1_award");
		$this->award1_rate = C("award1_rate");
		$this->award1_img = C("award1");
		
		$this->award2_award = C("award2_award");
		$this->award2_rate = C("award2_rate");
		$this->award2_img = C("award2");
		
		$this->award3_award = C("award3_award");
		$this->award3_rate = C("award3_rate");
		$this->award3_img = C("award3");
		
		$this->award4_award = C("award4_award");
		$this->award4_rate = C("award4_rate");
		$this->award4_img = C("award4");
		
		$this->award5_award = C("award5_award");
		$this->award5_rate = C("award5_rate");
		$this->award5_img = C("award5");
		
		$this->award6_award = C("award6_award");
		$this->award6_rate = C("award6_rate");
		$this->award6_img = C("award6");
		
		if(IS_POST){
			if($this->balance < $this->lottery_fee){
			    echo 'cjgo(' , $this->balance , ',0)';
		        exit;
			}
			
			if(C("lottery_count_per_day") > 0){
				$map["UG_account"] = $_SESSION['uname'];
				$map['UG_getTime'] = array('egt',date('Y-m-d 00:00:00', time()));
                $map['UG_getTime'] = array('elt',date('Y-m-d 23:59:59', time()));
				$map['UG_dataType'] = 'cj';
			    $tmp = M("userget")->where($map)->count();
				
				if($tmp >= C("lottery_count_per_day")){
				    echo 'cjgo(' , $this->balance , ',-1)';
		            exit;
				}
			}
			
			//扣钱
			$data[$col_name] = $this->balance - $this->lottery_fee;
			M('user')->where(array('UE_ID' => $_SESSION ['uid']))->save($data);
			unset($data);
			//进入总记录
			$data['UG_account'] = $this->user;
			$data['UG_money'] = "-".$this->lottery_fee;
			$data['UG_getTime'] = date("Y-m-d H:i:s", time());
			$data['UG_allGet'] = $this->balance;
			$data['UG_balance'] = $this->balance - $this->lottery_fee;
			$data['UG_dataType'] = 'cj';
			$data['UG_note'] = '抽奖扣除'.$this->award_wallet."钱包".$this->lottery_fee."金币";
			M('userget')->add($data);
			//抽奖
			$prizeConfig = array($this->award1_rate,
			                     $this->award2_rate,
								 $this->award3_rate,
								 $this->award4_rate,
								 $this->award5_rate,
			...
			$data['award_level'] = $winIndex + 1;
			$data['add_time'] = date("Y-m-d H:i:s", time());
			
			switch($winIndex){
				case 0:
					$data['award_detail'] = $this->award1_award;
					break;
				case 1:
					$data['award_detail'] = $this->award2_award;
					break;
				case 2:
					$data['award_detail'] = $this->award3_award;
					break;
				case 3:
					$data['award_detail'] = $this->award4_award;
					break;
				case 4:
					$data['award_detail'] = $this->award5_award;
					break;
				case 5:
					//再接再厉
					break;
			...
			echo 'cjgo(' , ($this->balance - $this->lottery_fee) , ',' , ($winIndex + 1) , ')';
		}else{
			$this->display();
		}
	}
	...
	public function task() {
		...
		$row = M('config')->where(array("name"=>"pd_content"))->find();
        $this->assign("content", $row["super_val"]);
		
		$map['if_type'] = '排单币任务';
		$rows = M('info')->where($map)->order('if_id')->select();
		$this->assign("v_list", $rows);
		$this->display( );
	}
	...
	public function xm() {
		...
		if (IS_AJAX) {
			...
			if (false) {
				...
				$username=trim($data_P ['dfzh']);
				$condition['UE_account'] = $username;
				//$condition['UE_phone'] = $username;
				$condition['_logic'] = 'OR';
				$addaccount = M ( 'user' )->where ($condition)->find ();
				if (!$addaccount) {
